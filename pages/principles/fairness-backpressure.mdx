...
<Callout type="info" title="Why This Matters">
  SP8D’s approach to fairness and backpressure makes high-performance
  concurrency “just click.” This page will show you how and why.
</Callout>
...
<Callout type="success" title="How to Read This Diagram">
...
```mermaid
flowchart TD
    P1[Producer 1] -- Request Slot --> Buffer
    P2[Producer 2] -- Request Slot --> Buffer
    C1[Consumer 1] -- Request Slot --> Buffer
    Buffer -- Grant Slot to Producer 1 --> P1
    Buffer -- Grant Slot to Producer 2 --> P2
    Buffer -- Grant Slot to Consumer 1 --> C1
```

<sub className="diagram-caption">
  <strong>Diagram:</strong> This schematic illustrates how SP8D's slot state
  machine grants access to producers and consumers in a fair, round-robin
  manner, preventing starvation and ensuring every agent gets a turn.
</sub>

<Callout type="info" title="Fairness in Action">
  SP8D's slot state machine ensures that all producers and consumers are granted
  access in a fair, round-robin manner, preventing starvation.
</Callout>
